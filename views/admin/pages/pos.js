...
function setupPOSListeners() {
    ...
    barcodeInput.onkeydown = function(e) {
        ...
        if (e.key === 'ArrowUp') {
            e.preventDefault();
            if (count === 0) return;
            selectedSuggestionIndex = selectedSuggestionIndex <= 0 ? count - 1 : selectedSuggestionIndex - 1;
            updateSuggestionHighlight(items);
            return;
        }
...
function addProductToCart(product) {
    ...
    if (existing) {
        ...
        else { showToast('No more stock available'); return; }
    } else {
        ...
        cart.push({
            ...
            quantity: 1, tax: 0, discount: 0, max_stock: product.stock
        });
    }
...
function renderCart() {
    ...
    if (!tbody) return;
    if (cart.length === 0) { tbody.innerHTML = ''; if (empty) empty.classList.remove('hidden'); return; }
    ...
    for (var i = 0; i < cart.length; i++) {
        ...
        html += '<td class="px-4 py-3"><p class="text-sm font-medium text-gray-900">' + escHtml(item.product_name) + '</p>';
        html += '<p class="text-xs text-gray-400">' + escHtml(item.sku || item.barcode || '') + '</p></td>';
        html += '<td class="px-4 py-3"><input type="number" value="' + item.selling_price + '" min="0" step="0.01" class="w-24 text-right px-2 py-1 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" onchange="updateItem(' + i + ', \'selling_price\', this.value)"></td>';
        html += '<td class="px-4 py-3"><div class="flex items-center justify-center gap-1">';
        html += '<button onclick="changeQty(' + i + ', -1)" class="w-6 h-6 rounded bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm flex items-center justify-center">-</button>';
        html += '<input type="number" value="' + item.quantity + '" min="1" max="' + item.max_stock + '" class="w-14 text-center px-1 py-1 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" onchange="updateItem(' + i + ', \'quantity\', this.value)">';
        html += '<button onclick="changeQty(' + i + ', 1)" class="w-6 h-6 rounded bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm flex items-center justify-center">+</button>';
        html += '</div></td>';
        html += '<td class="px-4 py-3"><input type="number" value="' + item.tax + '" min="0" max="100" step="0.01" class="w-16 text-right px-2 py-1 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" onchange="updateItem(' + i + ', \'tax\', this.value)"></td>';
        html += '<td class="px-4 py-3"><input type="number" value="' + item.discount + '" min="0" step="0.01" class="w-20 text-right px-2 py-1 border border-gray-200 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" onchange="updateItem(' + i + ', \'discount\', this.value)"></td>';
        html += '<td class="px-4 py-3 text-right text-sm font-semibold text-gray-900">Rs. ' + rt.toFixed(2) + '</td>';
        html += '<td class="px-4 py-3"><button onclick="removeItem(' + i + ')" class="text-red-400 hover:text-red-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></td>';
        html += '</tr>';
    }
...
function updateItem(index, field, value) {
    ...
    if (field === 'quantity') cart[index].quantity = Math.max(1, Math.min(cart[index].quantity, cart[index].max_stock));
    renderCart(); recalculate();
}

function changeQty(index, delta) {
    var newQty = cart[index].quantity + delta;
    if (newQty >= 1 && newQty <= cart[index].max_stock) { cart[index].quantity = newQty; renderCart(); recalculate(); }
}

function removeItem(index) { cart.splice(index, 1); renderCart(); recalculate(); }

function clearCart() {
    if (cart.length === 0) return;
    if (!confirm('Clear all items?')) return;
    cart = []; renderCart(); recalculate();
}

function recalculate() {
    var subtotal = 0, totalItemTax = 0, totalItemDiscount = 0;
    for (var i = 0; i < cart.length; i++) {
        var item = cart[i];
        var lineTotal = item.selling_price * item.quantity;
        subtotal += lineTotal;
    ...
    document.getElementById('discountDisplay').textContent = '- Rs. ' + (totalItemDiscount + globalDiscount).toFixed(2);
    document.getElementById('taxDisplay').textContent = 'Rs. ' + (totalItemTax + globalTax).toFixed(2);
    ...
    if (change > 0) {
        document.getElementById('changeDisplay').textContent = 'Rs. ' + change.toFixed(2);
...
function completeOrder() {
    ...
    var globalDiscount = parseFloat(document.getElementById('globalDiscount').value) || 0;
    var globalTaxRate = parseFloat(document.getElementById('globalTax').value) || 0;
    var subtotal = 0, totalItemTax = 0, totalItemDiscount = 0;
    for (var i = 0; i < cart.length; i++) {
        var item = cart[i];
        var lineTotal = item.selling_price * item.quantity;
    ...
    lastOrderData = {
        order_number: '', date: new Date().toLocaleString(),
        subtotal: subtotal, discount: totalItemDiscount + globalDiscount,
        tax: totalItemTax + globalTax, total: grandTotal,
        amount_paid: amountPaid, change: Math.max(0, amountPaid - grandTotal),
    ...
    for (var j = 0; j < cart.length; j++) {
        lastOrderData.items.push({
            barcode: cart[j].barcode, product_name: cart[j].product_name,
            selling_price: cart[j].selling_price, quantity: cart[j].quantity,
            tax: cart[j].tax, discount: cart[j].discount, total: calcRowTotal(cart[j])
        });
    }
    ...
    var orderData = {
        ...
        amount_paid: amountPaid, change_amount: lastOrderData.change,
        status: 'completed', notes: document.getElementById('orderNotes').value || null,
        items: []
    };
    for (var k = 0; k < cart.length; k++) {
        orderData.items.push({
            product_id: cart[k].product_id, product_name: cart[k].product_name,
            sku: cart[k].sku, barcode: cart[k].barcode,
            quantity: cart[k].quantity, selling_price: cart[k].selling_price,
            purchase_price: cart[k].purchase_price, tax: cart[k].tax,
            discount: cart[k].discount, total: calcRowTotal(cart[k])
        });
    }
    ...
    .then(function(result) {
        if (result.success) {
            ...
            document.getElementById('printOrderNumber').textContent = 'Sale #' + result.sale_number;
        ...
        } else { showToast(result.message || 'Failed to create order'); }
    })
    .catch(function() { showToast('Network error'); })
    .finally(function() { btn.disabled = false; btn.textContent = 'Save Sale'; });
}
...
function printReceipt() {
    ...
    for (var i = 0; i < d.items.length; i++) {
        var item = d.items[i];
        var itemLines = '<div class="item-row">' +
            '<div class="item-left">' +
            '<div class="item-name">' + escHtml(item.product_name) + '</div>' +
            '<div class="item-qty-price">' + item.quantity + ' x Rs. ' + item.selling_price.toFixed(2) + '</div>' +
            '</div>' +
            '<div class="item-right">Rs. ' + item.total.toFixed(2) + '</div>' +
            '</div>';
        if (item.discount > 0 || item.tax > 0) {
            var details = [];
            if (item.discount > 0) details.push('Disc: -Rs.' + item.discount.toFixed(2));
    ...
    if (d.amount_paid > 0) {
        paidSection =
            '<div class="summary-row"><span>Amount Paid</span><span>Rs. ' + d.amount_paid.toFixed(2) + '</span></div>' +
            '<div class="summary-row highlight-green"><span>Change</span><span>Rs. ' + d.change.toFixed(2) + '</span></div>';
    }

    var receiptHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' +
        ...
        'html, body { width: 80mm; font-family: "Courier New", monospace; font-size: 11px; color: #000; background: #fff; }' +
        ...
        '<div class="info-grid">' +
        '<div class="info-row"><span class="label">Receipt #</span><span class="value">' + escHtml(d.order_number) + '</span></div>' +
        '<div class="info-row"><span class="label">Date</span><span class="value">' + escHtml(d.date) + '</span></div>' +
        '<div class="info-row"><span class="label">Cashier</span><span class="value">' + escHtml(d.cashier) + '</span></div>' +
        '<div class="info-row"><span class="label">Payment</span><span class="value"><span class="payment-badge">' + escHtml(d.payment_method.toUpperCase()) + '</span></span></div>' +
        '</div>' +
        ...
        '<div class="summary">' +
        '<div class="summary-row"><span class="s-label">Subtotal</span><span class="s-value">Rs. ' + d.subtotal.toFixed(2) + '</span></div>' +
        (d.discount > 0 ? '<div class="summary-row discount"><span class="s-label">Discount</span><span class="s-value">- Rs. ' + d.discount.toFixed(2) + '</span></div>' : '') +
        (d.tax > 0 ? '<div class="summary-row"><span class="s-label">Tax</span><span class="s-value">Rs. ' + d.tax.toFixed(2) + '</span></div>' : '') +
        '<div class="summary-total"><span>TOTAL</span><span>Rs. ' + d.total.toFixed(2) + '</span></div>' +
        paidSection +
        '</div>' +
